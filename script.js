...
const logoIds = {
  Arsenal: 9825, 'Aston Villa': 10252, Bournemouth: 8678, Brentford: 9937,
  'Brighton and Hove Albion': 10204, 'Brighton & Hove Albion': 10204, Chelsea: 8455,
  'Crystal Palace': 9826, Everton: 8668, Fulham: 9879, 'Leeds United': 8463,
  Liverpool: 8650, 'Manchester City': 8456, 'Manchester United': 10260,
  'Newcastle United': 10261, 'Nottingham Forest': 10203, Sunderland: 8472,
  'Tottenham Hotspur': 8586, Tottenham: 8586, 'West Ham United': 8654,
  'Wolverhampton Wanderers': 8602, 'Real Madrid': 8633, Barcelona: 8634,
  'Atletico Madrid': 8639, 'Atlético Madrid': 8639, 'Bayern Munich': 9823,
  'Borussia Dortmund': 9789, 'Inter Milan': 8636, Inter: 8636, 'AC Milan': 8564,
  Juventus: 9885, 'Paris Saint-Germain': 9847, PSG: 9847
};
...
function teamBadge(team) {
  const initials = String(team || '?').replace(/\b(fc|cf|afc|united|city|club)\b/gi, '').trim().split(/\s+/).slice(0, 2).map((part) => part[0]).join('').toUpperCase() || '?';
...
function isLiveEvent(event) {
  return Boolean(event?.commence_time && new Date(event.commence_time).getTime() <= Date.now() && !event.completed);
}

async function getJson(path) {
  const response = await fetch(`${API_BASE}${path}`);
  const data = await response.json();
  if (!response.ok || data?.ok === false) throw new Error(data?.error || 'Unable to load football data');
  return data;
}
...
function matchCard(event, live = false) {
  ...
  const score = scores.length ? `${scores[0]?.score ?? 0} - ${scores[1]?.score ?? 0}` : '—';
...
async function loadFootballPage() {
  ...
  try {
    ...
    if (leagueSelect) {
      leagueSelect.innerHTML = '<option value="all">All active football</option>' + featured.map((league) => `<option value="${escapeHtml(league.key)}">${escapeHtml(league.title)}</option>`).join('');
      leagueSelect.addEventListener('change', () => loadFootballMatches(leagueSelect.value));
    }
    if (leagueGrid) {
      leagueGrid.innerHTML = featured.slice(0, 6).map((league, index) => `<article class="info-card league-card"><div class="league-logo">${String(index + 1).padStart(2, '0')}</div><h3>${escapeHtml(league.title)}</h3><p>${escapeHtml(league.description || 'Football competition')}</p><small>${escapeHtml(league.key)}</small></article>`).join('');
    }
...
async function loadFootballMatches(sport) {
  ...
  try {
    ...
    matchesGrid.innerHTML = events.length ? events.map((event) => matchCard(event)).join('') : '<div class="data-empty">No football fixtures are currently listed.</div>';
  } catch (error) {
...
async function loadLivePage() {
  ...
  try {
    ...
    const upcoming = events.filter((event) => !isLiveEvent(event) && !event.completed).sort((a, b) => new Date(a.commence_time) - new Date(b.commence_time)).slice(0, 12);
    liveGrid.innerHTML = live.length ? live.map((event) => matchCard(event, true)).join('') : '<div class="data-empty">No football matches are live right now.</div>';
    upcomingGrid.innerHTML = upcoming.length ? upcoming.map((event) => matchCard(event)).join('') : '<div class="data-empty">No upcoming football fixtures found.</div>';
    if (hero) hero.innerHTML = live.slice(0, 3).map((event) => `<div class="page-list-row"><div><strong>${escapeHtml(event.home_team)}</strong><small>${escapeHtml(event.sport_title || event.sport_key || 'Football')}</small></div><span class="page-score">${Array.isArray(event.scores) ? `${event.scores[0]?.score ?? 0}:${event.scores[1]?.score ?? 0}` : 'LIVE'}</span><div><strong>${escapeHtml(event.away_team)}</strong></div></div>`).join('') || '<div class="page-list-row"><div><strong>No live football</strong><small>We will update this panel automatically</small></div><span>—</span></div>';
